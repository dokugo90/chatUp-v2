...
/*{roomName: "Kitchen Chat", lastMessage: "What's for dinner tonight?", lastMessageTime: "11/02 17:30", roomId: "AbCdEfG1HjKlMnOpQrS"},
    ...
    /*if (process.browser) {
                  const loggedInChatUp = localStorage.getItem("loggedInChatUp");
                  setLoggedIn(loggedInChatUp === "true");
                }*/
    // Create New Room Code Copy and paste if needed back

     /*
  addDoc(createdRoomsCollection, {})
  .catch(error => {
    alert("Try Again");
  });*/
  ...
  /* {
            ...
            // TODO: RoomId File Code If needed  !Important

            /*export default function chatRoom({ pathId, pathRoom }) {
    // eslint-disable-next-line react-hooks/rules-of-hooks
    const [chats, setChats] = useState([
      {roomName: "Kitchen Chat", lastMessage: "What's for dinner tonight?", lastMessageTime: "11/02 17:30", roomId: "AbCdEfG1HjKlMnOpQrS", members: [{email: "dokugo90@gmail.com"}]},
    ])
    ...
    function findCurrentIndex(e) {
      ...
      <title>{chats[pathId].roomName}</title>
          ...
          : <></>
            ...
            {
              filteredChats.map((item, index) => (
                item.members.some((user) => user.email === email) ? (
                  <>
                  <Link scroll={false} href="/[id]/[roomid]" as={`/${findCurrentIndex()}/${item.roomId}`}>
                    <div onMouseOver={() => setCurrentPath(item.roomId)}>
                      <Dashboard
...
// TODO: Other RoomId file Code if needed
